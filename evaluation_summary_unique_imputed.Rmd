...
B20_0609 <- B20_0609 %>% mutate(sum = rowSums(across(where(is.logical))))
B20_0609 <- B20_0609$sum>=TRESHHOLD
B20_0630 <- dplyr::select(data, contains("20"))
B20_0630 <- B20_0630 %>% mutate(sum = rowSums(across(where(is.logical))))
B20_0630 <- B20_0630$sum>=TRESHHOLD
...
############## B 20-30-40
############## B
data <- readRDS(file = "results_v002/results_run1_mqpar_extracts_2gether_E4_D1_B_MBR_LFQ_combined_txt_proteinGroups.txt_Unique.RDS")
data <- data[[1]]
B <- dplyr::select(data, contains("Pool_20"))
B20 <- B
B20 <- B20 %>% mutate(sum = rowSums(across(where(is.logical))))
B20 <- B20$sum>=TRESHHOLD
B <- dplyr::select(data, contains("Pool_30"))
B30 <- B
B30 <- B30 %>% mutate(sum = rowSums(across(where(is.logical))))
B30 <- B30$sum>=TRESHHOLD
#b40
B40 <- dplyr::select(data, contains("Pool_40"))
B40 <- B40 %>% mutate(sum = rowSums(across(where(is.logical))))
B40 <- B40$sum>=TRESHHOLD
...
AF <- AF %>% mutate(sum = rowSums(across(where(is.logical))))
AF <- AF$sum>=TRESHHOLD
AF <- as.data.frame(cbind(AF, AF_TF_raw$FASTA))
TF <- dplyr::select(AF_TF_raw, contains("TF"))
TF <- TF %>% mutate(sum = rowSums(across(where(is.logical))))
TF <- TF$sum>=TRESHHOLD
...
B <- B %>% mutate(sum = rowSums(across(where(is.logical))))
B <- B$sum>=TRESHHOLD
...
ggsave("./pics/unique_AF_TF_B_imputed.png")
...
aj <- aj %>% mutate(sum = rowSums(across(where(is.logical))))
aj <- aj$sum>=TRESHHOLD
...
ggsave("./pics/unique_B_D1pool_imputed.png")
...
for (i in seq(1:10)){
  ...
  #use M as selector 
  tmp <- tmp %>% mutate(sum = rowSums(across(where(is.logical))))
  aj[,paste(ltters[i],"M",sep="")] <- tmp$sum>=TRESHHOLD
...
```{r}
# compare with pool
aj_pool <- aj_pool %>% mutate(sum = rowSums(across(where(is.logical))))
aj_pool <- aj_pool$sum>=TRESHHOLD
aj_pool <- aj_pool*1
...
data_D2_pool <- data_D2_pool %>% mutate(sum = rowSums(across(where(is.logical))))
data_D2_pool <- data_D2_pool$sum>=TRESHHOLD
data_D2_fractions <- dplyr::select(data, contains("20"))
data_D2_fractions <- data_D2_fractions %>% mutate(sum = rowSums(across(where(is.logical))))
data_D2_fractions <- data_D2_fractions$sum>=TRESHHOLD
...
ggvenn(lst,
       ...
       stroke_size = 0.5, set_name_size = 5, text_size = 4, digits = 0, fill_alpha = 0.9)
ggsave("./pics/unique_D2__imputed.png")
...
D1 <- D1 %>% mutate(sum = rowSums(across(where(is.logical))))
D1 <- D1$sum>=TRESHHOLD
...
D2 <- D2 %>% mutate(sum = rowSums(across(where(is.logical))))
D2 <- D2$sum>=TRESHHOLD
...
ggsave("./pics/unique_D1_D2_imputed.png")
```
