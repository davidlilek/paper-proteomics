...
D1 <- rowMeans(D1,na.rm = TRUE)
D1 <- as.data.frame(cbind(D1, FASTA))
D2_raw <- readRDS(file = "results_v002/results_run1_mqpar_extracts_2gether_D2_LFQ_combined_txt_proteinGroups.txt_LFQ.RDS")
D2_raw <- D2_raw[[1]]
FASTA <- D2_raw$FASTA
D2 <- dplyr::select(D2_raw, contains("Pool_20"))
D2 <- as.data.frame(lapply(D2, as.numeric))
D2 <- rowMeans(D2,na.rm = TRUE)
D2 <- as.data.frame(cbind(D2, FASTA))
...
D1_D2_comparison <- as.data.frame(lapply(D1_D2_comparison, as.numeric))
D1_D2_comparison$q <- D1_D2_comparison$D1/D1_D2_comparison$D2

plot(D1_D2_comparison$q,ylab="Quotient Intensität D1/D2",xlab="Protein")
abline(h=1,col="red")
sum(D1_D2_comparison$q<1, na.rm = TRUE)
sum(D1_D2_comparison$q>1, na.rm = TRUE)
hist((D1_D2_comparison$D2-D1_D2_comparison$D1)/rowMeans(D1_D2_comparison[,c(1:2)],na.rm=TRUE)*100,
     main="Relative Abweichung Intensität zwischen D2/D1",
     xlab="Intensitäts-Abweichung D2/D1 [%]")
summary((D1_D2_comparison$D2-D1_D2_comparison$D1)/rowMeans(D1_D2_comparison[,c(1:2)],na.rm=TRUE)*100)
```
