...
---
```{r}
# thresshold for consecutive values
TRESHHOLD <- 2
```
...
rows <- apply(B20_0609, 1, function(x)
  as.numeric(any(rle(x)$lengths >= TRESHHOLD & rle(x)$values)))
#transform 0 and 1 values to true and false
B20_0609 <- rows==1
B20_0630 <- dplyr::select(data, contains("20"))
rows <- apply(B20_0630, 1, function(x)
  as.numeric(any(rle(x)$lengths >= TRESHHOLD & rle(x)$values)))
#transform 0 and 1 values to true and false
B20_0630 <- rows==1
...
B20 <- B
rows <- apply(B20, 1, function(x)
  as.numeric(any(rle(x)$lengths >= TRESHHOLD & rle(x)$values)))
#transform 0 and 1 values to true and false
B20 <- rows==1
B <- dplyr::select(data, contains("Pool_30"))
B30 <- B
rows <- apply(B30, 1, function(x)
  as.numeric(any(rle(x)$lengths >= TRESHHOLD & rle(x)$values)))
#transform 0 and 1 values to true and false
B30 <- rows==1
#b40
B40 <- dplyr::select(data, contains("Pool_40"))
rows <- apply(B40, 1, function(x)
  as.numeric(any(rle(x)$lengths >= TRESHHOLD & rle(x)$values)))
#transform 0 and 1 values to true and false
B40 <- rows==1
...
ggsave("./pics/unique_AF_TF_B_imputed_imputed_2consecutivevalues.png")
...
ggsave("./pics/unique_B_D1pool_imputed_imputed_2consecutivevalues.png")
...
for (i in seq(1:10)){
  ...
  #use M as selector 
  rows <- apply(tmp, 1, function(x)
  as.numeric(any(rle(x)$lengths >= TRESHHOLD & rle(x)$values)))
  tmp <- tmp %>% mutate(sum = rowSums(across(where(is.logical))))
  aj[,paste(ltters[i],"M",sep="")] <- rows==1
...
```{r}
# compare with pool
rows <- apply(aj_pool, 1, function(x)
  as.numeric(any(rle(x)$lengths >= TRESHHOLD & rle(x)$values)))
#transform 0 and 1 values to true and false
aj_pool <- rows==1
aj_pool <- aj_pool*1
...
rows <- apply(data_D2_pool, 1, function(x)
  as.numeric(any(rle(x)$lengths >= TRESHHOLD & rle(x)$values)))
#transform 0 and 1 values to true and false
data_D2_pool <- rows==1
data_D2_fractions <- dplyr::select(data, contains("20"))
data_D2_fractions <- data_D2_fractions[,c(1,2,5:20,3,4)]
rows <- apply(data_D2_fractions, 1, function(x)
  as.numeric(any(rle(x)$lengths >= TRESHHOLD & rle(x)$values)))
#transform 0 and 1 values to true and false
data_D2_fractions <- rows==1
...
ggvenn(lst,
       ...
       stroke_size = 0.5, set_name_size = 5, text_size = 4, digits = 0, fill_alpha = 0.9)
ggsave("./pics/unique_D2__imputed_2consecutivevalues.png")
...
ggsave("./pics/unique_D1_D2_imputed_imputed_2consecutivevalues.png")
```
