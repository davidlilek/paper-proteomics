...
* Bande	Größenbereich [kDa]	Intensität
  + B1	> 300	- 
  + B2	250	~
  + B3	200	-
  + B4	150	+
...
```{r}
lines <- data.frame(
  value = c(300,250,200,150,60,80,40,60,30,40,20,25,10,15),
  group = c("B1",
            "B2",
            "B3",
            "B4",
...
lines$value <- lines$value*1000

ggplot(dat_4plot_sub, aes(x=variable, y=Mass)) + 
  geom_boxplot() +
  geom_boxplot(data=lines,aes(factor(group),value),
               inherit.aes=FALSE,color="red",size=0.5)
```
...
data[] <- data > 0
data[is.na(data)]<- FALSE  
data_sub <- data[,-ncol(data)]
B_D1 <- data_sub*1
B_D1 <- sapply(seq(1,19,by=2),function(i) rowSums(B_D1[,i:(i+1)]))
B_D1[B_D1==0] <- NA

df <- as.data.frame(B_D1)
#change columns because B1,B10,B2,B3,...
df <- df[,c(1,3,4,5,6,7,8,9,10,2)]
colnames(df) <- paste("B",1:10,sep="")
df$FASTA <- FASTA
...
#getsequences <- GetSequences(fasta)
#saveRDS(getsequences,file="sequences_uniprot_E4_D2_B_LFQ_combined_txt_proteinGroups.txt_Unique.RDS")
getsequences <- readRDS("sequences_uniprot_E4_D2_B_LFQ_combined_txt_proteinGroups.txt_Unique.RDS")
df$Mass <- getsequences$Mass
df$number <- 1:length(data$FASTA)
dat_4plot <- melt(df[,-c(11)], id.vars = c("Mass","number"))
dat_4plot <- na.omit(dat_4plot)
...
#saveRDS(seq,file="fasta_comp_sharedproteins_proteinmasses_E4_D1_B.RDS")
seq <- readRDS("fasta_comp_sharedproteins_proteinmasses_E4_D1_B.RDS")
for (i in 1:nrow(data_sub)){
  x<-rep(1:9,each=2)
  y<-t(data_sub[i,-ncol(data_sub)])[,1]
  modell <- lm(y~x)
  #slope
  slope <- c(slope,modell$coefficients[2])
  r2 <- c(r2,summary(modell)$r.squared)
  plot(rep(1:9,each=2),
...
* lineare Regression wurde die Daten gelegt und die Steigung & R2 hier geplottet
