...
# Figure 4 - Electrophoretic separation 
...
# Figure 5 - Biological annotation
...
extract_goterm <- function(input_string) {
  # extract first string between square brackets
...
# reorder category
df$Category <- factor(df$Category, levels = c("D1 vs. D2", "2D vs. 3D", "D1: frac vs. total", "D2: frac vs. total"))
...
p <- ggplot(df, aes(x = Category, y = Percentage, fill = Variable)) +
  ...
  theme(legend.position = "bottom", legend.box = "horizontal",
        ...
        strip.text = element_text(margin = margin(20, unit = "pt"))) +
...
df <- data.frame(
  Category = rep(tmp, times = 3),
  Type = c(rep(c("biological process", "molecular function", "cellular compartment"), each = 9)),
  Percentage = c(round(results_comp_bp[-c(4:6)]*100,1),
                 round(results_comp[-c(4:6)]*100,1),
                 round(results_comp_cc[-c(4:6)]*100,1)),
  Variable = rep(c("B", "A", "C",
                   "H", "G", "I",
                   "K", "J", "L"),3)
  )
...
# reorder category
df$Category <- factor(df$Category, levels = c("D1 vs. D2", "D1: frac vs. total", "D2: frac vs. total"))

# vector for desired order
desired_order <- c("A", "G", "J",
                   "B", "H", "K",
                   "C", "I", "L")
...
# Figure 6 - Bioinformatic Data Analysis
