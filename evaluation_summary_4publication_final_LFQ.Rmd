...
res_raw <- readRDS(file = "results_v003/mqpar_extracts_2gether_reanalysisCM_extraktejakob_proteinGroups.txt_LFQ.RDS")
res_raw <- res_raw[[1]]
...
data_sub <- as.data.frame(data$LFQ.noMBR)
...
# D1-D2 comparison
# prepare data D1
D1_raw <- readRDS(file = "results_v002/results_run1_mqpar_extracts_2gether_E4_D1_a-j_LFQ_combined_txt_proteinGroups.txt_LFQ.RDS")
D1_raw <- D1_raw[[1]]
D1 <- dplyr::select(D1_raw, contains("Pool"))
D1$LFQ.intensity.E4_D1_Pool_01 <- as.logical(D1$LFQ.intensity.E4_D1_Pool_01)
D1$LFQ.intensity.E4_D1_Pool_02 <- as.logical(D1$LFQ.intensity.E4_D1_Pool_02)
D1 <- apply(D1, 1, any)
fasta <- c()
...
D2 <- as.data.frame(lapply(D2, as.logical))
D2 <- apply(D2, 1, any)
fasta <- c()
...
data_raw <- readRDS("X:/wissenschaftliche Veröffentlichungen/Publikationen/2022/Proteomics_JRS_DL/Data/27_20230201_FH/results_v002/_LFQ.RDS")
data <- data_raw[[1]]
data_clear <- dplyr::select(data, contains("_b"))
D1 <- data_clear$LFQ.intensity.1_b_01 + data_clear$LFQ.intensity.1_b_02
D1_b <- data_clear$LFQ.intensity.2_b_01 +
  data_clear$LFQ.intensity.2_b_02
D2 <- data_clear$LFQ.intensity.4_b_01 + data_clear$LFQ.intensity.4_b_02
data_clear <- as.data.frame(cbind(D1, D1_b, D2))
data_clear[data_clear==0] <- NA
data_clear[data_clear<0] <- 1
...
print(c("D1","D1_b","D2"))
sum(!is.na(data_clear$D1))
sum(!is.na(data_clear$D1_b))
sum(!is.na(data_clear$D2))
```
...
D1_raw <- D1_raw[[1]]
D1 <- dplyr::select(D1_raw, contains("Pool"))
D1 <- as.data.frame(lapply(D1, as.logical))
D1 <- apply(D1, 1, any)
...
data <- readRDS("X:/wissenschaftliche Veröffentlichungen/Publikationen/2022/Proteomics_JRS_DL/Data/27_20230201_FH/results_v002/_LFQ.RDS")
data <- data[[1]]
...
AF <- as.data.frame(lapply(AF, as.logical))
AF <- apply(AF, 1, any)
fasta <- c()
...
TF <- as.data.frame(lapply(TF, as.logical))
TF <- apply(TF, 1, any)
...
B <- as.data.frame(lapply(B, as.logical))
B <- apply(B, 1, any)
fasta <- c()
...
aj <- as.data.frame(lapply(aj, as.logical))
aj <- apply(aj, 1, any)
fasta <- c()
...
data_D2_pool <- as.data.frame(lapply(data_D2_pool, as.logical))
data_D2_pool <- apply(data_D2_pool, 1, any)
data_D2_fractions <- dplyr::select(data, contains("20"))
data_D2_fractions <- as.data.frame(lapply(data_D2_fractions, as.logical))
...
for(x in 1:9) {
  i <- i+1
  tmp <- apply(B[,c(i,i+1)], 1, any)
  dat <- c(dat,sum(tmp*1, na.rm=TRUE))
  i <- i+1
}
dat <- as.data.frame(
  cbind(dat,
...
for(x in 1:10) {
  i <- i+1
  tmp <- apply(data_D2_fractions[,c(i,i+1)], 1, any)
  dat <- c(dat,sum(tmp*1, na.rm=TRUE))
  i <- i+1
}
dat <- as.data.frame(
  cbind(dat,
        c("B1", "B10", "B2", "B3", "B4", "B5", "B6", "B7", "B8", "B9")
))
colnames(dat) <- c("no","fraction")
dat$no <- as.numeric(dat$no)
# re-arrange position
...
```{r}
#result_df <- subset(D1, !is.na(D1))
#D1_uni <- GetProteinGOInfo(result_df$fasta)
#saveRDS(D1_uni,"./go_terms/graph2_E4_uni_LFQ.RDS")
GO_1 <- readRDS("./go_terms/graph2_E4_uni_LFQ.RDS")
#result_df <- subset(D2, !is.na(D2))
#D2_uni <- GetProteinGOInfo(result_df$fasta)
#saveRDS(D2_uni,"./go_terms/graph2_E5_uni_LFQ.RDS")
GO_2 <- readRDS("./go_terms/graph2_E5_uni_LFQ.RDS")
################################# molecular function
...
#result_df <- subset(comparison_2D_3D, !is.na(E4))
#D2_uni <- GetProteinGOInfo(result_df$fasta)
#saveRDS(D2_uni,"./go_terms/graph2_D1_uni_LFQ.RDS")
GO_1 <- readRDS("./go_terms/graph2_D1_uni_LFQ.RDS")
#result_df <- subset(comparison_2D_3D, !is.na(E5))
#D3_uni <- GetProteinGOInfo(result_df$fasta)
#saveRDS(D3_uni,"./go_terms/graph2_D2_uni_LFQ.RDS")
#D2_uni <- GetProteinGOInfo(D2)
#saveRDS(D2_uni,"./go_terms/graph2_D2_uni.RDS")
GO_2 <- readRDS("./go_terms/graph2_D2_uni_LFQ.RDS")
...
```{r}
#result_df <- subset(B, !is.na(B))
#B <- GetProteinGOInfo(result_df$fasta)
#saveRDS(B,"./go_terms/graph3_B_B_uni_LFQ.RDS")
#result_df <- subset(aj, !is.na(aj))
#aj <- GetProteinGOInfo(result_df$fasta)
#saveRDS(aj,"./go_terms/graph3_B_aj_uni_LFQ.RDS")
GO_1 <- readRDS("./go_terms/graph3_B_B_uni_LFQ.RDS")
GO_2 <- readRDS("./go_terms/graph3_B_aj_uni_LFQ.RDS")
...
#}
#data_D2$fasta <- fasta
#result_df <- subset(data_D2, !is.na(data_D2_pool))
#pool <- GetProteinGOInfo(result_df$fasta)
#saveRDS(pool,"./go_terms/graph3_C_pool_uni_LFQ.RDS")
#result_df <- subset(data_D2, !is.na(data_D2_fractions))
#fraction <- GetProteinGOInfo(result_df$fasta)

#saveRDS(fraction,"./go_terms/graph3_C_fractions_uni_LFQ.RDS")
GO_1<- readRDS("./go_terms/graph3_C_pool_uni_LFQ.RDS")
GO_2 <- readRDS("./go_terms/graph3_C_fractions_uni_LFQ.RDS")
